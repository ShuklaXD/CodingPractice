/*Professor Jain has a class full of notorious students. To get anything done from them is a herculean task. Prof Jain wanted to organize a test. He gave this responsibility to Paul. Paul did an excellent job of organizing the test. As a reward, the professor gave him an sequence of numbers to play with. But Paul likes playing with "interesting" sequence of numbers, which are sequences that have equal elements.
...
What’s the minimum number of coins Prof Jain needs to turn his sequence into a “interesting" sequence for Paul?
Input Format
...
int main() {
    ...
    int min_cost = INT_MAX, cost = INT_MAX;
